...
module File = struct
  ...
  let file path (action : unit -> (file_state, unit) I.Action.t) =
  ...
  let file' path ?(user = `Current) ?group ?(perms = 0o644) ?(make_dirs = true) contents =
    ...
    { user; group = Option.value ~default:user group; make_dirs; perms; contents }
...
module Directory = struct
  ...
  let directory path (action : unit -> (dir_state, unit) I.Action.t) =
  ...
  let directory' path ?(user = `Current) ?group ?(perms = 0o755)
    ...
    let open I.Action in
    let+ () = action () in
    { user; group = Option.value ~default:user group; perms }
end
